...
ax2 = lp.init_fig('z (mm)', '$n^{-1}$ (m$^{3}$)', label_size=16)
...
# Scale the lengths to agree at high signal levels
L2 = L1 * I2[2] / I1[2]
...
U = (75./21.) * (fg_pre + o2_pre)
# Approximate the voltage
V = 20. * 11. / 12.
# Approximate ion density
n1 = csprobe.cs2_n(V, 1e-3 * I1/L1, U, .000254)
n2 = csprobe.cs2_n(V, 1e-3 * I2/L2, U, .000254)
C = np.polyfit(Z[index],1./n1[index],1)
c0 = C[1]
c1 = C[0]
...
ax2.plot(Z, 1./n1, 'ko', mec='k', mfc='w', label='Peak')
ax2.plot(Z, 1./n2, 'ks', mec='k', mfc='w', label='Center')
ax2.legend(loc=0)
...
print('fg_pre_scfh %.4f'%fg_pre)
print('o2_pre_scfh %.4f'%o2_pre)
print('fg_post_scfh %.4f'%fg_post)
print('o2_post_scfh %.4f'%o2_post)
print('total_scfh %.4f'%(fg_pre + o2_pre))
print('ratio_fo %.6f'%(fg_pre / o2_pre))
print('c0 %.8e'%c0)
print('c1 %.8e'%c1)
print('Uc1 %.8e'%(c1 * U))
